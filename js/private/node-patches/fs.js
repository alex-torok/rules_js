...
const patcher = (fs = _fs, roots) => {
    ...
    fs.readlink = (...args) => {
        ...
        args[args.length - 1] = (err, str) => {
            ...
            if (escapedRoot) {
                let next = nextHop(str);
                if (!next) {
                    if (next == undefined) {
                        // The escape from the root is not mappable back into the root; throw EINVAL
                        let err = new Error(`ENOENT: no such file or directory, realpath '${args[0]}'`);
                        err.errno = -2;
                        ...
                        // The escape from the root is not mappable back into the root; throw EINVAL
                        let err = new Error(`EINVAL: invalid argument, readlink '${args[0]}'`);
                        err.errno = -22;
                        err.syscall = 'readlink';
                        err.code = 'EINVAL';
                        err.path = args[0];
    ...
    function guardedReadLinkSync(start, escapedRoot = undefined) {
        let loc = start;
        let next = nextHop(loc);
        if (!next) {
    ...
    function guardedRealPathSync(start, escapedRoot = undefined) {
        ...
        for (let loc = start, next;; loc = next) {
            ...
            if (!next) {
                ...
                let err = new Error(`ENOENT: no such file or directory, realpath '${start}'`);
                err.errno = -2;
